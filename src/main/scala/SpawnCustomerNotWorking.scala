...
// end of animation handling
  ...
  stateReg := spawnSecondCustomer
    ...
    is(spawnSecondCustomer){
      ...
      when(customerXPosVec(1) <= customerXPosVec(0) - 50.S || customerXPosVec(0) <= customerXPosVec(1) - 50.S){
        customerXPosVec(1) := Mux(customerXPosVec(1) >= 400.S, customerXPosVec(1) - 50.S, customerXPosVec(1) + 50.S)
        when(customerSeatYVec(1) === customerSeatYVec(0)){
