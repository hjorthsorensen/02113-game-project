...
class beerLeftFSM extends Module{
   ...
   def doubleDabble(score: UInt): (UInt, UInt) = {
    val shiftRegInit = Cat(0.U(8.W), score(15, 0)) // 16bit digits + 16bit input
    var shiftReg = shiftRegInit
    
    for (i <- 0 until 8) {
      val bcd = shiftReg(25, 16)
