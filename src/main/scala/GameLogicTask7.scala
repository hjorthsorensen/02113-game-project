...
class GameLogicTask7(SpriteNumber: Int, BackTileNumber: Int) extends Module {
  ...
  /////////////////////////////////////////////////////////////////       
  val idle :: compute1 :: compute2 ::calculateCol:: done :: Nil = Enum(5)
  ...
  val sprite4XReg = RegInit(230.S(11.W))
  val sprite4YReg = RegInit((360-60).S(10.W))
  ...
  val spritePlayerAnimationVisibilityReg = RegInit(true.B)
  val bubbleVisibleReg = RegInit(true.B)

  //Making sprite 0 visible
  io.spriteVisible(0) := spritePlayerAnimationVisibilityReg
  ...
  io.spriteVisible(4) := bubbleVisibleReg


  //Connecting resiters to the graphic engin
  ...
  switch(stateReg) {
    ...
    is(compute2) {
      ...
      when(sprite4YReg <= 50.S){
        ...
        bubbleVisibleReg := true.B
      }
      
      stateReg := calculateCol
      }

    is(calculateCol) {
      when((sprite0XReg - sprite4XReg).abs < 24.S && (sprite0YReg - sprite4YReg).abs < 16.S) {
        // Collision detected between sprite 0 and sprite 1
        // You can add your collision handling logic here
        bubbleVisibleReg := false.B
      }
